...
static char* __get_wayland_display(int argc, char **argv)
{
	int i = 0;
	for (i = 0; i < argc; i++) {
		if (argv[i] && g_strcmp0(argv[i],
				AUL_K_WAYLAND_DISPLAY) == 0) {
...
static char* __get_xdg_working_dir(int argc, char **argv)
{
	int i = 0;
	for (i = 0; i < argc; i++) {
		if (argv[i] && g_strcmp0(argv[i],
				AUL_K_WAYLAND_WORKING_DIR) == 0) {
			if (i + 1< argc && argv[i + 1])
				return argv[i + 1];
		}
	}
...
static void __init_wayland(int argc, char **argv)
{
	char *wayland_disp;
	char *xdg_working_dir;

	wayland_disp = __get_wayland_display(argc, argv);
	xdg_working_dir = __get_xdg_working_dir(argc, argv);
