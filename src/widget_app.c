...
EXPORT_API widget_class_h widget_app_add_class_provider(widget_class_h widget_class, const char *class_id,
		widget_instance_lifecycle_callback_s callback, void *user_data)
{
	if (class_id == NULL) {
...
EXPORT_API int widget_app_class_id(widget_class_h widget_class, char **class_id)
{
	if (widget_class == NULL || class_id == NULL)
		return WIDGET_ERROR_INVALID_PARAMETER;

	*class_id = widget_class->classid;

	return WIDGET_ERROR_NONE;
}

EXPORT_API int widget_app_foreach_class_provider(widget_class_provider_cb callback, void *user_data)
{
	widget_class_h handle = NULL;
	...
	handle = class_provider;

	while (handle) {
		if (callback(handle->classid, handle, user_data) < 0)
			break;
