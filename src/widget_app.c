...
static int __instance_resume(widget_class_h handle, const char *id, bundle *b, int send_update)
{
...
static int __instance_pause(widget_class_h handle, const char *id, bundle *b, int send_update)
{
	widget_context_s *wc = __find_context_by_id(id);
	int ret;
...
static int __instance_create(widget_class_h handle, const char *id, bundle *b)
{
	...
	handle->ops.create(wc, wc->content, w, h, handle->user_data);
	ret = __send_update_status(handle->classid, wc->id,
			WIDGET_INSTANCE_EVENT_CREATE, wc->content, 0);

	return ret;
}

static int __instance_destroy(widget_class_h handle, const char *id,
		widget_app_destroy_type_e reason, bundle *b, int send_update)
{
	widget_context_s *wc = __find_context_by_id(id);
	int ret = 0;
	...
	handle->ops.destroy(wc, reason, b,
			handle->user_data);

	if (reason == WIDGET_DESTROY_TYPE_PERMANENT) {
		event = WIDGET_INSTANCE_EVENT_DESTROY;
	} else {
		ret = __send_update_status(handle->classid, id,
				WIDGET_INSTANCE_EXTRA_UPDATED, b, 0);
	}

	ret = __send_update_status(handle->classid, id, event, b, 0);
...
static void __control(bundle *b)
{
	...
	if (strcmp(operation, "create") == 0) {
		...
		__instance_destroy(handle, id, WIDGET_APP_DESTROY_TYPE_PERMANENT, b, UPDATE_SEND);
	} else if (strcmp(operation, "resume") == 0) {
		__instance_resume(handle, id, b, UPDATE_SEND);
	} else if (strcmp(operation, "pause") == 0) {
		__instance_pause(handle, id, b, UPDATE_SEND);
	} else if (strcmp(operation, "terminate") == 0) {
		__instance_destroy(handle, id, WIDGET_APP_DESTROY_TYPE_TEMPORARY, b, UPDATE_SEND);
	}
...
static void __pause_all(int send_update)
{
	...
	while (iter != NULL) {
		...
		switch (cxt->state) {
		case WC_READY:
			instance_resume(cxt->provider, cxt->id, NULL, send_update);
			instance_pause(cxt->provider, cxt->id, NULL, send_update);
			break;
		case WC_RUNNING:
			instance_pause(cxt->provider, cxt->id, NULL, send_update);
			break;
...
static void __destroy_all(int reason, int send_update)
{
	...
	while (iter != NULL) {
		...
		switch (cxt->state) {
		case WC_PAUSED:
			instance_destroy(cxt->provider, cxt->id, reason, cxt->content, send_update);
			break;
		}
		iter = g_list_next(iter);
	}
...
static Eina_Bool __show_cb(void *data, int type, void *event)
{
	...
	if (cxt)
		__instance_resume(cxt->provider, cxt->id, NULL, UPDATE_SEND);
	else
...
static Eina_Bool __hide_cb(void *data, int type, void *event)
{
	...
	if (cxt)
		__instance_pause(cxt->provider, cxt->id, NULL, UPDATE_SEND);
	else
		LOGE("unknown window error: %d", ev->win);
...
static void __after_loop()
{
	...
	__pause_all(UPDATE_MUTE);
	__destroy_all(WIDGET_APP_DESTROY_TYPE_TEMPORARY, UPDATE_SEND);
...
static gboolean __finish_event_cb(gpointer user_data)
{
	...
	switch (wc->state) {
	case WC_READY:
		__instance_resume(wc->provider, wc->id, wc->content, MUTE);
	case WC_RUNNING:
		__instance_pause(wc->provider, wc->id, wc->content, MUTE);
	case WC_PAUSED:
		__instance_destroy(wc->provider, wc->id,
				WIDGET_DESTROY_TYPE_TEMPORARY, wc->content, 1);
		break;
	default:
		break;
	}
